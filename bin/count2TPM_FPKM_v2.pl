...
while(<IN>){
	...
	my @tmp = split /\t/;
	my $id = (split (/\./, $tmp[0]))[0];
	$ensg_len{$id}{'anno'} = join("\t", @tmp[1..7]);
	$ensg_len{$id}{'length'} = $tmp[-1];
}
close IN;
...
print OUT_TPM "GID\tSymbol\tGene_Type\tChr\tStart\tEnd\tStrand\tGene_Length\t" . join ("\t",@all_sample) . "\n";
print OUT_FPKM "GID\tSymbol\tGene_Type\tChr\tStart\tEnd\tStrand\tGene_Length\t" . join ("\t",@all_sample) . "\n";
#for my $g (@all_gene){
for my $g (sort {$a cmp $b} keys %all_gene){
	print OUT_TPM "$g\t$ensg_len{$g}{'anno'}";
	print OUT_FPKM "$g\t$ensg_len{$g}{'anno'}";
	for my $s(@all_sample){
